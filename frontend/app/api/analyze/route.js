...
// Forwards POST /api/analyze to `${NEXT_PUBLIC_API_BASE || http://localhost:8001}/api/analyze`
export async function POST(req) {
  try {
    const body = await req.text(); // keep raw body
    const upstreamBase =
      process.env.NEXT_PUBLIC_API_BASE || "https://alpha.struct.inc";
    const url = `${upstreamBase.replace(/\/+$/, "")}/api/analyze`;
